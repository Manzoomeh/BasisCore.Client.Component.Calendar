...
export class UiDatePickerMobile {
  ...
  generateDaysUiAppModel(parentElement: HTMLElement): void {
    ...
    const submitDate: HTMLElement = wheelDatePicker.querySelector(
      "[data-bc-datepicker-mobile-submit]"
    );
    const days = wheelDatePicker.querySelector(
      "[data-bc-datepicker-mobile-days-wrapper]"
    );
    const months = wheelDatePicker.querySelector(
      "[data-bc-datepicker-mobile-months-wrapper]"
    );
    const years = wheelDatePicker.querySelector(
      "[data-bc-datepicker-mobile-years-wrapper]"
    );
    const dayList = document.createElement("ul");
    const monthList = document.createElement("ul");
    const yearList = document.createElement("ul");
    const dateRangeList = wheelDatePicker.querySelector(
      "[data-bc-datepicker-daterange-list]"
    ) as HTMLElement;
    const toadyNum = 6;
    ...
    dateRangeList.addEventListener("touchend", (e) => {
      ...
      if (click == true) {
        dateRangeList.querySelectorAll("li").forEach((currentLi, index) => {
          const currentLiAttr = currentLi.getAttribute(
            "data-bc-datepicker-mobile-wheel-curent"
          );
        ...
        if (rangeUnit < 0) {
          dateRangeList.style.transform = `translate(0px,-${
            currentPositionRange + 30
        ...
        dateRangeList
          .querySelectorAll("li")
          [rangeIndex].setAttribute(
            "data-bc-datepicker-mobile-wheel-curent",
            "true"
          );
        this.selectRange = Number(
          dateRangeList
            .querySelectorAll("li")
            [rangeIndex].getAttribute("data-range")
        );
      }
      click = false;
    });
    dayList.addEventListener("touchend", (e) => {
      ...
      if (click == true) {
        ...
        if (dayUnit < 0) {
          dayList.style.transform = `translate(0px,-${
            currentDayPosition + unit * 30
          }px)  scale(1) translateZ(0px)`;
    ...
    monthList.addEventListener("touchend", (e) => {
      ...
      if (click == true) {
        ...
        if (monthUnit < 0) {
          monthList.style.transform = `translate(0px,-${
            currentMonthPosition + unit * 30
          }px)  scale(1) translateZ(0px)`;
    ...
    yearList.addEventListener("touchend", (e) => {
      ...
      if (click == true) {
        ...
        if (yearUnit < 0) {
          yearList.style.transform = `translate(0px,-${
            currentYearPosition + unit * 30
          }px)  scale(1) translateZ(0px)`;
        ...
        yearList
          .querySelectorAll("li")
          [yearIndex].setAttribute(
            "data-bc-datepicker-mobile-wheel-curent",
            "true"
          );
        this.selectYear = Number(
          yearList.querySelectorAll("li")[yearIndex].textContent
        );
      }
      click = false;
    });
