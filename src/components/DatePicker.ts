...
import { DayNumber, DayValue, MonthNumber, MonthValue } from "./type-alias";
...
export class DatePicker {
  ...
  protected createMountHeader(): Node {
    ...
    if (this.monthValues.length == 1) {
      ...
      nextButton.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="10" height="16" viewBox="0 0 10 16" fill="none">
      <path d="M1 1L9 8L1 15" stroke="#5F5F5F" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>`;
      prevButton.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="10" height="16" viewBox="0 0 10 16" fill="none">
      <path d="M9 1L1 8L9 15" stroke="#5F5F5F" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>`;
      ...
      refreshButton.setAttribute("data-datepicker-refresh", "");
    } else {
    ...
    if (this.options.switchType) {
      ...
      changeTypeButton.addEventListener("click", (e) => {
        ...
        if (this.options.culture == "en") {
          var convertDate: DayValue = this.dateUtil.convertToJalali(
            this.months[this.activeIndex].value
          );
        } else {
          var convertDate: DayValue = this.dateUtil.convertToGregorian(
            this.months[this.activeIndex].value
          );
        }
