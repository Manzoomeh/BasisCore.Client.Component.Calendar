...
export class Month {
  ...
  public constructor(owner: DateRange | DatePicker, value: MonthValue) {
    ...
    if (
      this.range.options.secondCulture &&
      this.range.options.secondCulture == "en"
    ) {
      this.secondMonthName = this.range.dateUtil.getGregorianMonthsName(value);
    } else if (
      this.range.options.secondCulture &&
      this.range.options.secondCulture == "fa"
    ) {
    ...
    this.currentYear = this.range.dateUtil.getCurrentYear(
      firstDay,
      this.range.options.culture
    );
